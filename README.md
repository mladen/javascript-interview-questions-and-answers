...
>4. **null** for unknown values – a standalone type that has a single value null. (**<span style="color: red">Null is also considered an [object](https://www.toptal.com/javascript/interview-questions)!</span>**)
...
Two way data binding can cause side effects, that are difficult to debug. **<span style="color:red">What side effects??!</span>**

One way data flow means that there is a single source of truth (**<span style="color:red">What is the single source of truth? Definition/explanation!</span>**) and changes in that single source of truth can only flow in one direction.
